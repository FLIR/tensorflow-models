...
def densenet(inputs,
             final_layer = None, # Added to get layers before prediction for SSD hook up.
             num_classes=1000,
             reduction=None,
             growth_rate=None,
             num_filters=None,
  ...
  #j: To get better control over the grabbed layer names make a dictionary
  # to pull from later if final_layer is set.
  end_dict = {}
  if data_format == 'NCHW':
  ...
  end_points_collection = 'final' + '_end_points'
  with slim.arg_scope([slim.batch_norm, slim.dropout],
    ...
    for i in range(num_dense_blocks - 1):
      dense_name = 'dense_block' + str(i+1)
      transition_name = 'transition_block' + str(i+1)
      # dense blocks
      net, num_filters = _dense_block(net, num_layers[i], num_filters,
                                      growth_rate,
                                      scope= dense_name)
      #j: Add them to dictionary.
      end_dict[dense_name] = net 
      # Add transition_block
      net, num_filters = _transition_block(net, num_filters,
                                            compression=compression,
                                            scope=transition_name)
      #j: Add them to dictionary.
      end_dict[transition_name] = net
      #j: Check if you're at final_layer
      # the is the option to pull before a transition block 
      # but I don't think that is a good idea.
      if final_layer in [dense_name,transition_name]: break 

    if final_layer is not None: return net, end_dict
...
#j: Added final_layer argument
def densenet121(inputs,final_layer=None, num_classes=1000, data_format='NHWC', is_training=True, reuse=None):
  return densenet(inputs, final_layer=final_layer,
